...
import { Branch, ErrorResponse, PaginatedResponse, PaginationParams, GeoAddress, EntityInput } from 'shared/types';
import { MESSAGES } from 'shared/constants';
import { mapBranch, mapBranches, mapBranchRetrievalModel, mapError, getFullAddress, createProblemDetails } from 'shared/helpers';
...
export const fetchBranchesTotal = createAsyncThunk<PaginatedResponse<Branch> | undefined, void, { rejectValue: ProblemDetailsModel }>(
  ...
  async (_, { rejectWithValue }) => {
    ...
    return foldClientResult(
      result,
      (response) => mapPaginatedResponse(response, mapBranchRetrievalModel),
...
>('branch/fetchBranches', async ({ pageNumber, pageSize, search }, { getState, rejectWithValue }) => {
  ...
  return foldClientResult(
    result,
    (response) => {
      const data = mapPaginatedResponse(response, mapBranchRetrievalModel);
...
>('branch/fetchBranchesByIds', async (ids, { rejectWithValue }) => {
  const query = new BranchQueryRequestModel(ids, '', null, null);
  const result = await branchClient.getBranchesAsync(query, new AbortController().signal);
...
export const fetchBranchById = createAsyncThunk<
  Branch,
  { id: string | Branch['id']; skipState?: boolean; shouldFetchBranchGeoAddress?: boolean },
  { rejectValue: ProblemDetailsModel }
>('branch/fetchBranchById', async ({ id, shouldFetchBranchGeoAddress = true }, { getState, dispatch, rejectWithValue }) => {
  try {
    ...
    return foldClientResult(
      result,
      async (data) => {
        const branch = mapBranchRetrievalModel(data);
        const state = getState() as RootState;
        const timeZones = state.license.system.item?.entitlements.timeZones || [];
        const countries = state.license.system.item?.entitlements.countries || [];
        const companyCountryCode = state.company.company.item!.countryCode;
        const countryName = countries.find((country) => country.code === branch.address?.countryCode)?.name;
        const fullAddress = branch.address ? getFullAddress({ ...branch.address, countryName }, false) : '';
...
export const deleteBranch = createAsyncThunk<void, Branch['id'], { state: RootState; rejectValue: ProblemDetailsModel }>(
  ...
  async (id, { dispatch, getState, rejectWithValue }) => {
    return foldClientUnitResult(
      await branchClient.deleteBranchAsync(id, new AbortController().signal),
      () => {
        dispatch(resetBranchesFetchStatus());
