...
import { BaseEntity, EntityInput, ErrorResponse, Module, SubModule } from 'shared/types';
...
const EntityManager = <T extends BaseEntity, TInput extends BaseEntity>({
  ...
  React.useEffect(() => {
    const shouldFetch = !hasFetched.current && id && (!values || values.id !== BigInt(id)) && fetchEntityAction;
