...
const EmployeeEditPage: React.FC = () => {
  ...
  const handleSuccess = () => {
    ...
    if (profile && 'id' in profile && employee && profile.id === employee.id) {
  ...
  const isBranchOptionAvailable = assignedBranches.some((branchItem) => branchItem.id === employee?.assignedBranchId);
  const branchItems =
    employee?.assignedBranchId && !isBranchOptionAvailable
      ...
      : assignedBranches;
  const isDepartmentOptionAvailable = assignedDepartments.some((departmentItem) => departmentItem.id === employee?.assignedDepartmentId);
  const departmentItems =
    employee?.assignedDepartmentId && !isDepartmentOptionAvailable
      ...
      : assignedDepartments;
  const isManagerOptionAvailable = managers.some((managerItem) => managerItem.id === employee?.reportsToId);
  const managerItems =
